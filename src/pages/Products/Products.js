  ...
  return (
    <><Paper elevation={2} sx={{ maxHeight: '200px', position: 'fixed' }}>
      ...
      <div className="divFilters">
        <input placeholder="precio minimo SOLO NUMEROS"
          onChange={(event) => {
            let priceData = event.target.value;
            if (priceData) { setLowPrice(priceData); }
          }
          }
        />
        <input placeholder="precio maximo SOLO NUMEROS"
          onChange={(event) => {
            let priceData = event.target.value;
            if (priceData) { setHighPrice(priceData); }
          }
          }
        />
        <input placeholder="provincia SELECT"
          onChange={(event) => {
        ...
        />
        <input placeholder="estado SELECT"
          onChange={(event) => {
        ...
        <button onClick={() => navigate(`/products${splitedUrl}&lowPrice=${lowPrice}&highPrice=${highPrice}&province=${province}&status=${status}`)}>
          Filtrar
        </button>
      </div>
    </Paper>
